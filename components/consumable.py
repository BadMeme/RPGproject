...
class Consumable(BaseComponent):
    ...
    def get_action(self, consumer: Actor) -> Optional[actions.Action]:
        """try to return the action for this itme."""
...
class MagicDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target:
            self.engine.message_log.add_message(
                f"A dart of arcane force strikes the {target.name} for {self.damage} damage."
            )
...
class ConfusionConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if not target:
            raise Impossible("You Must select and enemy to target.")
