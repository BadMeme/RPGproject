...
item_chances: Dict[int, List[Tuple[Entity, int]]] ={
    ...
    4: [(gameobjects.magebolt_scroll, 25), (gameobjects.sword, 5)],
    6: [(gameobjects.mageburst_scroll, 25), (gameobjects.chain_mail, 15)],
}
...
def generate_dungeon( #TODO this feels like it needs a different name
        max_rooms: int,
        room_min_size: int,
        room_max_size: int,
        map_width: int,
    ...
    for r in range (max_rooms):
        ...
        if any(new_room.intersects(other_room) for other_room in rooms):
            continue # this room intersects, so go to the next attempt.
                #Room is valid if no intersections
        
        dungeon.tiles[new_room.inner] = tile_types.floor

        if len(rooms) == 0:
            #The first room, where player currently starts.
            player.place(*new_room.center, dungeon)
        else: # All rooms after first
            # Make connecting tunnel to previos room from this one
